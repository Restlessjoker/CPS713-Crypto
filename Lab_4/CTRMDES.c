...
int main(){
	...
	*/
	...
	if (encrypt){
		...
		ctrNum = (len+7)/8; 	//Number of Counters to encrypt 
		...
		for ( index = 0; index < ctrNum; index++){
			sprintf(ctrP, "%d", CTR+index);
			printf("\tCOUNTER ENCRYPTED: %s  \n", ctrP);
			encryptAlg(ctrP, roundKeys, &cText[pos]);
			pos+=8;
		}

		//XOR the result from encrypting the counters with the plaintext
		for (index = 0; index < pos; index++)
			cpText[index] = pText[index]  ^ cText[index];
		cpText[pos] = '\0';
		...
		printf("Please enter ciphertext (MAX: 500 chars): ");
		scanf("\n");
		...
		for ( index = 0; index < ctrNum; index++){
			sprintf(ctrC, "%d", CTR+index);
			printf("\tCOUNTER ENCRYPTED: %s  \n", ctrC);
			encryptAlg(ctrC, roundKeys, &pText[pos]);
			pos+=8;
		}

		//XOR the result from encrypting the counters with the ciphertext
		for (index = 0; index < pos; index++)
			ccText[index] = cText[index]  ^ pText[index];
		ccText[pos] = '\0';
		//Remove additional padding so they do not appear on screen
		if (padding){
			pads = pos - len;
